...
update_x-ui() {
    ...
    ${curl_bin} -fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/KianMythic/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz 2> /dev/null
    if [[ $? -ne 0 ]]; then
    ...
    ${curl_bin} -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/KianMythic/3x-ui/main/x-ui.sh > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        ...
        ${curl_bin} -fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/KianMythic/3x-ui/main/x-ui.rc > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
        ...
        if [ -f "x-ui.service" ]; then
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/KianMythic/3x-ui/main/x-ui.service.debian > /dev/null 2>&1
                        ;;
                    arch | manjaro | parch)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/KianMythic/3x-ui/main/x-ui.service.arch > /dev/null 2>&1
                        ;;
                    *)
                        ${curl_bin} -fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/KianMythic/3x-ui/main/x-ui.service.rhel > /dev/null 2>&1
                        ;;
                esac
